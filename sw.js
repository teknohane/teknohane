...
// Not 2: Uygulama artık GitHub Pages'te (/teknohane/ alt yolu) VE Firebase Hosting'te (kök dizin)
// AYNI dosyalarla yayınlanıyor — bu yüzden hiçbir path SABİT (hardcoded) değil, hepsi
// self.registration.scope üzerinden DİNAMİK hesaplanıyor (SW'nin kaydolduğu gerçek yola göre).
...
function appShellUrls() {
  ...
  return {
    scope,
    shellHtml: scope + 'index.html',
    list: [scope, scope + 'index.html', scope + 'manifest.json', scope + 'teknohane.png'],
  };
}
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then((cache) => cache.addAll(appShellUrls().list))
...
self.addEventListener('fetch', (event) => {
  ...
  const { scope, shellHtml } = appShellUrls();
    ...
    event.respondWith(
      fetch(req)
        .then((res) => {
          ...
          caches.open(CACHE_NAME).then((cache) => cache.put(shellHtml, resClone));
          return res;
        })
        .catch(() =>
          caches.match(shellHtml).then((cached) => cached || caches.match(req))
        )
    );
    return;
  }
